...
function Navbar() {
  ...
  return (
    <>
      <motion.nav
        className={`fixed w-full top-0 left-0 z-50 px-4 transition-all duration-500 ${
      ...
      >
        ...
        <div className={`max-w-3xl w-full transition-all duration-500 relative overflow-hidden ${
          isScrolled 
                ...
                <li key={link.id} className="cursor-pointer">
                  ...
                  >
                    <span className={`relative z-10 transition-colors duration-300 ${
                      activePage === link.id ? 'text-[#FF00CD]' : ''
                    }`}>{link.name}</span>
                    ...
                    {activePage === link.id && (
                      <motion.span 
                        className={`absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-to-r from-[#FF00CD] to-[#734EFF] ${
                          isScrolled ? 'shadow-[0_0_8px_rgba(255,0,205,0.6)]' : ''
                        }`}
          ...
          <div className="md:hidden">
            <div className="flex items-center justify-between h-16">
              ...
              <div className={`font-bold text-lg bg-clip-text text-transparent bg-gradient-to-r from-[#FF00CD] to-[#734EFF] transition-opacity duration-300 ${
            ...
            <motion.div 
              className={`overflow-hidden px-2 ${
                isMenuOpen ? 'bg-black/80 backdrop-blur-md rounded-xl mt-2 border border-white/10' : ''
              }`}
              initial={{ height: 0 }}
                ...
                {navLinks.map((link) => (
                  <li key={link.id} className="cursor-pointer">
                    <a 
                      ...
                      className={`block py-2.5 text-center transition-all ${
                        activePage === link.id 
